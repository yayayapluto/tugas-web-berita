        ...
        <?php if(count($Berita->tampilkan_berita_terbaru()) > 0): ?>
            <?php foreach($Berita->tampilkan_berita_terbaru() as $berita): ?>
              <div class="flex items-center p-4 bg-white rounded shadow hover:shadow-md">
                ...
                <div class="flex-grow">
                  <h4 class="text-md font-bold mb-3 text-ellipsis text-nowrap overflow-hidden w-40 lg:w-96"><?= str_replace("-"," ",$berita['judul'])?></h4>
                  <a href="/web_berita/baca/<?= $berita['judul']?>" class="py-1 px-3 bg-blue-600 rounded-sm text-white hover:shadow-lg shadow-md">Selengkapnya</a>
                </div>
              </div>
            <?php endforeach ?>
            <?php else: ?>
